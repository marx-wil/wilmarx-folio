...
const ThemeChanger = ({ onThemeChange }) => {
  ...
  const [currentIcon, setCurrentIcon] = useState(colorMode === "light" ? "moon" : "sun");
  ...
  const handleThemeChange = () => {
    ...
    tl.fromTo(overlay, {
      ...
      clipPath: `circle(${Math.hypot(window.innerWidth, window.innerHeight)}px at ${rippleX}px ${rippleY}px)`,
      duration: 0.8,
      ease: "power3.inOut",
      onComplete: () => {
        toggleColorMode();
